...
Muy = torch.tensor([[0, 1, 1], [0, 0, 0], [1, 0, 0], [0, 1, 0]])
Muy = Muy.float()

Mud = torch.tensor([[0, 1, 1], [0, 0, 0], [1, 0, 0], [0, 1, 0]])
Mud = Mud.float()
...
ctl = Controller(sys.f, sys.n_agents, Muy, Mud, sys.n, sys.m, n_xi, l)
...
m1 = np.zeros((2, 8))
m2 = np.hstack((np.eye(4), np.zeros((4, 4))))
m3 = np.zeros((2, 8))
m4 = np.hstack((np.zeros((4, 4)), np.eye(4)))

m = np.array([m1, m2, m3, m4])

m = np.vstack([m1, m2, m3, m4])

d = np.diag(np.arange(1, 13))
test = m.T @ d @ m
itest = np.linalg.inv(test + 2 * np.diag(np.ones(8)))

itestcopia = m.T @ np.linalg.inv(d+2*np.eye(12)) @ m
test2 = m @ itest @ m.T
